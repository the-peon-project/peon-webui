...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow PEON project domains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*the-peon-project\.io$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@the-peon-project.org" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@the-peon-project.org" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
